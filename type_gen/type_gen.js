...
if (!settings.outputFile) {
  console.warn("No output file specified. Please specify an output file in the settings.");
  return;
}
...
if (process.env.ROOT_DIR && fs.existsSync(process.env.ROOT_DIR + '/config.json')) {
  let config = JSON.parse(fs.readFileSync(process.env.ROOT_DIR + '/config.json', 'utf8'));
  packsPath = './';
  gametestsPath = path.normalize(process.env.ROOT_DIR + '/' + config.regolith.dataPath + '/gametests/');
}

if (!fs.existsSync(gametestsPath)) {
  console.warn("Could not find gametests directory. Please make sure the gametests directory is present in the dataPath");
  return;
}
...
function loadLangFile(filePath) {
  ...
  return syncedLines.map(x => x.trim().split('=', 2).map(y => y.trim()));
}
...
// List entities
let enumContent = createEnumFromJsonFiles('Entities', obj => getValueByPath(obj, 'minecraft:entity/description/identifier'), packsPath + 'BP/entities/');

// List items
enumContent += createEnumFromJsonFiles('Items', obj => getValueByPath(obj, 'minecraft:item/description/identifier'), packsPath + 'BP/items/');

// List blocks
enumContent += createEnumFromJsonFiles('Blocks', obj => getValueByPath(obj, 'minecraft:block/description/identifier'), packsPath + 'BP/blocks/');

// List sounds
if (fs.existsSync(packsPath + 'RP/sounds/sound_definitions.json')) {
  let obj = loadJsonFile(packsPath + 'RP/sounds/sound_definitions.json')['sound_definitions'];
  let pairs = {};
...
if (fs.existsSync(packsPath + 'RP/texts/en_US.lang')) {
  ...
  for (const [key, value] of obj) {
    pairs[toTitleCase(key.split('.').map(x => x.includes(':') ? x.split(':', 2)[1] : x).join('.'))] = key;
  }
...
if (fs.existsSync(packsPath + 'BP/loot_tables')) {
  let obj = readdirSyncRecursive(packsPath + 'BP/loot_tables').map(x => path.relative(packsPath + 'BP/loot_tables', x).replace(/\\/g, '/'));
  let pairs = {};
...
// List particles
enumContent += createEnumFromJsonFiles('Particles', obj => getValueByPath(obj, 'particle_effect/description/identifier'), packsPath + 'RP/particles/');
